...
begin
  while (input = $stdin.gets)
    ...
    action = if my_energy <= 3 # MUTATION: was: action = if my_energy <= 2
             ...
             elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 1 && my_energy >= 7 # MUTATION: was: elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 3 && my_energy >= 6
               # Opportunistic attack on very weak agents
               { action: 'attack', target: adjacent_info[:agents].first[:direction] }
             elsif my_energy < replication_threshold && rand < 0.7 # MUTATION: was: elsif my_energy < replication_threshold && rand < 0.8
