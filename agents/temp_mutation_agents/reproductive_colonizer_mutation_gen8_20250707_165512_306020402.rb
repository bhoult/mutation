...
begin
  while (input = $stdin.gets)
    ...
    action = if my_energy <= 2
             ...
             elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 3 && my_energy >= 3 # MUTATION: was: elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 3 && my_energy >= 6
