...
begin
  while (input = $stdin.gets)
    ...
    action = if my_energy <= 2
             ...
             elsif crowded && !adjacent_info[:empty].empty? && my_energy >= 8 # MUTATION: was: elsif crowded && !adjacent_info[:empty].empty? && my_energy >= 6
             ...
             elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 3 && my_energy >= 9 # MUTATION: was: elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 3 && my_energy >= 6
               # Opportunistic attack on very weak agents
               { action: 'attack', target: adjacent_info[:agents].first[:direction] }
             elsif my_energy < replication_threshold && rand < 0.6 # MUTATION: was: elsif my_energy < replication_threshold && rand < 0.8
               # Build energy for next replication
               { action: 'rest' }
             elsif !adjacent_info[:empty].empty? && rand < 0.2 # MUTATION: was: elsif !adjacent_info[:empty].empty? && rand < 0.3
