...
begin
  while (input = $stdin.gets)
    ...
    action = if my_energy <= 4 # MUTATION: was: action = if my_energy <= 3 # MUTATION: was: action = if my_energy <= 2
             ...
             elsif crowded && !adjacent_info[:empty].empty? && my_energy >= 6
             ...
             elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 5 && my_energy >= 10 # MUTATION: was: elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 2 && my_energy >= 7 # MUTATION: was: elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 3 && my_energy >= 6
               # Opportunistic attack on very weak agents
               { action: 'attack', target: adjacent_info[:agents].first[:direction] }
             elsif my_energy < replication_threshold && rand < 0.8
               # Build energy for next replication
               { action: 'rest' }
             elsif !adjacent_info[:empty].empty? && rand < 0.1 # MUTATION: was: elsif !adjacent_info[:empty].empty? && rand < 0.3
