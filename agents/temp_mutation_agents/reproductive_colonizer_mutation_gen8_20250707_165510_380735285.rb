...
begin
  while (input = $stdin.gets)
    ...
    action = if my_energy <= 2
             ...
             elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 3 && my_energy >= 5 # MUTATION: was: elsif adjacent_info[:agents].size == 1 && adjacent_info[:agents].first[:energy] <= 3 && my_energy >= 6
               ...
               { action: 'rest' }
             elsif !adjacent_info[:empty].empty? && rand < 0.4 # MUTATION: was: elsif !adjacent_info[:empty].empty? && rand < 0.3
