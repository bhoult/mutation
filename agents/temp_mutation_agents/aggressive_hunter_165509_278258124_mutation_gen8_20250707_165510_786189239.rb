...
begin
  while (input = $stdin.gets)
    ...
    action = if my_energy <= 1
             ...
             elsif my_energy >= 12 && rand < 0.5 # MUTATION: was: elsif my_energy >= 12 && rand < 0.4
               # High energy - sometimes replicate to create hunting pack
               { action: 'replicate' }
             elsif !empty_directions.empty? && rand < 0.9 # MUTATION: was: elsif !empty_directions.empty? && rand < 0.7 # MUTATION: was: elsif !empty_directions.empty? && rand < 0.6
