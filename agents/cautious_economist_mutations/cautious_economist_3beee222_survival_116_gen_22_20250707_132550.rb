...
# Content Hash: 3beee222
# Created: 2025-07-07 13:25:50
# Survival Time: 116 ticks
# Generation: 22
# World Size: 2x2
...
begin
  while (input = $stdin.gets)
    ...
    action = if my_energy <= critical_energy
               ...
               { action: 'replicate' }
             elsif my_energy < comfortable_energy && energy_delta >= 0
               # Building reserves successfully - continue
               { action: 'rest' }
             elsif !opportunities[:empty].empty? && energy_delta < -1 && rand < 0.4
