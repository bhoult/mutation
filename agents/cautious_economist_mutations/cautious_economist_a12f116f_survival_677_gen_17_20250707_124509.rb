...
# =============================================================================
# Original Agent: cautious_economist
# Content Hash: a12f116f
# Created: 2025-07-07 12:45:09
# Survival Time: 677 ticks
# Generation: 17
# World Size: 190x51
# Total Agents: Unknown
# Simulation Delay: 0.05
# Final Agent Count: 1
# =============================================================================
...
begin
  while (input = $stdin.gets)
    ...
    action = if my_energy <= critical_energy
               ...
               { action: 'replicate' }
             elsif my_energy < comfortable_energy && energy_delta >= 2 # MUTATION: was: elsif my_energy < comfortable_energy && energy_delta >= 0
               # Building reserves successfully - continue
               { action: 'rest' }
             elsif !opportunities[:empty].empty? && energy_delta < -1 && rand < 0.2 # MUTATION: was: elsif !opportunities[:empty].empty? && energy_delta < -1 && rand < 0.4
    ...
    if last_action == 'attack'
      if energy_delta > 1 # MUTATION: was: if energy_delta > 0
        new_successful += 1
      else
        new_failed += 1
      end
