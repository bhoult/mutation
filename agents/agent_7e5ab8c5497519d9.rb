# Fingerprint: 7e5ab8c5497519d9
# Created: 2025-07-05T22:17:51Z
# Parent: a5d0b522bebfa937
# Generation: 3
#!/usr/bin/env ruby
...
def choose_action(world_state, memory)
  ...
  if my_energy <= personality['death_threshold'] && (memory['energy_history'][-3..-1] || []).all? { |e| e <= personality['death_threshold'] + 1 }
  ...
  elsif target_info['energy'] >= 1 && my_energy >= 1 && target_info['energy'] > my_energy && rand < personality['aggression']
    ...
    if target_info['energy'] >= 1 && my_energy >= 1 && rand < personality['aggression'] * 0.7
      action = { action: 'attack', target: target_direction }
      memory['attacks_made'] = (memory['attacks_made'] || 0) + 1
    else
      action = { action: 'rest' }
